...
public class BlockChandelierRT extends BlockFallingBase {

	public BlockChandelierRT() {
		super(Material.IRON, "chandelier_brass");
		this.setHardness(2F);
	...
	private boolean suspended(World worldIn, BlockPos pos) {
		...
		if (state.getBlock() == ModBlocksRT.CHAIN_BRASS && state.getValue(BlockChainRT.AXIS) == EnumFacing.Axis.Y) {
			return true;
		}
